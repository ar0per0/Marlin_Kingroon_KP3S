...
void menu_advanced_settings();
  ...
  void menu_tool_offsets() {
    ...
    EDIT_ITEM_FAST_N(float42_52, Z_AXIS, MSG_HOTEND_OFFSET_A, &hotend_offset[1].z, Z_PROBE_LOW_POINT, 10.0, _recalc_offsets);
      ...
      CONFIRM_ITEM(MSG_STORE_EEPROM, MSG_STORE_EEPROM, MSG_BUTTON_CANCEL, ui.store_settings, nullptr, GET_TEXT_F(MSG_STORE_EEPROM));
    #endif
    END_MENU();
  }
#endif
...
void menu_configuration() {
  ...
  if(!busy){
    #if ENABLED(EEPROM_SETTINGS)
      CONFIRM_ITEM(MSG_STORE_EEPROM, MSG_STORE_EEPROM, MSG_BUTTON_CANCEL, ui.store_settings, nullptr, GET_TEXT_F(MSG_STORE_EEPROM));
      CONFIRM_ITEM(MSG_LOAD_EEPROM, MSG_LOAD_EEPROM, MSG_BUTTON_CANCEL, ui.load_settings, nullptr, GET_TEXT_F(MSG_LOAD_EEPROM));
    #endif

    CONFIRM_ITEM(MSG_RESTORE_DEFAULTS, MSG_RESTORE_DEFAULTS, MSG_BUTTON_CANCEL, ui.reset_settings, nullptr, GET_TEXT_F(MSG_RESTORE_DEFAULTS));
  }

  STATIC_ITEM_F(F("Marlin " SHORT_BUILD_VERSION));      //Marlin Version
  END_MENU();
}
